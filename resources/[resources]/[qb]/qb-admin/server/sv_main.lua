...
function addLog(message)
  ...
  local Execute = SQL("INSERT INTO adminmenu_log(identifier , log , name , date) VALUES(@identifier , @log , @name , @date)",{["@identifier"]=license,['@log']=message,["@name"]=GetPlayerName(source) , ['@date'] = os.time()})
...
AddEventHandler('qb-admin:server:SpawnItem', function(ItemName, ItemAmount)
  ...
  if CoreName.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    ...
    local Execute = SQL("INSERT INTO adminmenu_log(identifier , log , name , date) VALUES(@identifier , @log , @name , @date)",{["@identifier"]=license,['@log']='Spawned Item: '..ItemName..' ( '..CoreName.Shared.Items[ItemName].label..' ) | Amount: '..ItemAmount,["@name"]=GetPlayerName(source) , ['@date'] = os.time()})
  end
end)
...
AddEventHandler('qb-admin:server:Csay', function(msg)
  ...
  if CoreName.Functions.HasPermission(src, 'admin') or CoreName.Functions.HasPermission(src, 'god') then
    ...
    local Execute = SQL("INSERT INTO adminmenu_log(identifier , log , name , date) VALUES(@identifier , @log , @name , @date)",{["@identifier"]=license,['@log']='cSaid: '..msg,["@name"]=GetPlayerName(source) , ['@date'] = os.time()})
  end
end)
...
AddEventHandler('qb-admin:server:bringPlayer', function(target , pos)
  ...
  if CoreName.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    ...
    local Execute = SQL("INSERT INTO adminmenu_log(identifier , log , name , date) VALUES(@identifier , @log , @name , @date)",{["@identifier"]=license,['@log']='Bringed Player | ID: '..target..' | Display Name: '..GetPlayerName(target),["@name"]=GetPlayerName(source) , ['@date'] = os.time()})
  end
end)
...
AddEventHandler("qb-admin:server:setFav", function(data)
  ...
  if CoreName.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    ...
    local result =  SQL("UPDATE adminmenu_admin SET favorite = @fav WHERE cid = @cid" , { ["@fav"] = json.encode(data),["@cid"] = cid})
  end
end)
...
AddEventHandler("playerConnecting", function(playerName, setKickReason, deferrals)
  ...
  local data = SQL('SELECT * FROM adminmenu_banned WHERE license = @license' , {['@license'] = license})
  if data ~= nil then
    if(data[1] == nil) then
      deferrals.done()
    else
      ...
      if(os.time() > tonumber(data[1].length)) then
          deferrals.done()
          SQL('DELETE FROM adminmenu_banned WHERE license = @license' , {['@license'] = license})
      else
...
CoreName.Functions.CreateCallback('qb-admin:GetPlayerData', function(source, cb)
  ...
  local result =  SQL("SELECT * FROM adminmenu_admin WHERE cid = @cid" , {["@cid"] = cid})
  if(result[1] == nil) then
    SQL("INSERT INTO adminmenu_admin (cid , favorite) VALUES(@cid , @favorite)",{["@cid"]=cid,['@favorite']=json.encode({})})
  end
  cb(result)
end)
---------------------------------------------------------------------------------------------
CoreName.Functions.CreateCallback('qb-admin:GetBannedPlayers', function(source, cb)
  local src = source                                            
  if CoreName.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    local result =  SQL("SELECT * FROM adminmenu_banned" , {})
    cb(result)
  end
end)
---------------------------------------------------------------------------------------------
CoreName.Functions.CreateCallback('qb-admin:GetLogs', function(source, cb)
  local src = source                                            
  if CoreName.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    local result =  SQL("SELECT * FROM adminmenu_log" , {})
    cb(result)
  end
end)
...
AddEventHandler("qb-admin:BanPlayer", function(target , reason , length)
  ...
  if CoreName.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    ...
    for k,v in pairs(GetPlayerIdentifiers(source))do        
      ...
      local Execute = SQL("INSERT INTO adminmenu_banned(steam , reason , length , ip , discord , license, DisplayName) VALUES(@steam , @reason , @length , @ip , @discord, @license , @name)",{["@steam"]=steamid,['@reason']=reason,["@length"]=length,["@ip"]=ip , ['@discord'] = discord , ['license']= license , ['@name'] = GetPlayerName(target)})
      SQL("INSERT INTO adminmenu_log(identifier , log , name , date) VALUES(@identifier , @log , @name , @date)",{["@identifier"]=license,['@log']='Banned player: '..GetPlayerName(target)..' | Reason: '..reason..' | Banned till: '..os.date("%Y-%m-%d %H:%M",length)..'',["@name"]=nameofshit, ['@date'] = os.time()})
